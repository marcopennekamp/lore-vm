...
pub fn run<'a>(function: &Function<'a>, mut locals: Vec<u64>) {
    ...
    while inst_index < inst_count {
        ...
        match inst {
            ...
            &Instruction::Load(ref format) => {
                stack[stack_top] = locals[format.var as usize];
                stack_top += 1;
            },

            &Instruction::Store(ref format) => {
                stack_top -= 1;
                locals[format.var as usize] = stack[stack_top];
            },

            &Instruction::Add(ref format) => {
